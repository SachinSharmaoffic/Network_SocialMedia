    ...
    fetch('/' + username + '/follow', {
        ...
        .then(() => {
            if (origin === 'suggestion') {
                element.parentElement.innerHTML = `<button class="btn btn-success merabutton" type="button"   onclick="unfollow_user(this,'${username}','suggestion')">Following</button>`;
            }
            else if (origin === 'edit_page') {
                element.parentElement.innerHTML = `<button class="btn btn-success float-right merabutton" onclick="unfollow_user(this,'${username}','edit_page')"   id="following-btn">Following</button>`;
            }
...
function unfollow_user(element, username, origin) {
    ...
    fetch('/' + username + '/unfollow', {
        ...
        .then(() => {
            if (origin === 'suggestion') {
                element.parentElement.innerHTML = `<button class="btn merabutton btn-outline-success " type="button" onclick="follow_user(this,'${username}','suggestion')">Follow</button>`;
            }
            else if (origin === 'edit_page') {
                element.parentElement.innerHTML = `<button class="btn btn-outline-success merabutton  float-right"  onclick="follow_user(this,'${username}','edit_page')" id="follow-btn">Follow</button>`;
            }
            else if (origin === 'dropdown') {
                ///////////////////////////////////////////////////////////////////////////////////////////
            }
    ...
    fetch('/n/post/' + parseInt(post_id) + '/comments')
        ...
        .then(() => {
            setTimeout(() => {
                ...
                comment_div.style.overflow = 'hidden';
            }, 500);
        });
}
...
function display_comment(comment, container, new_comment = false) {
    ...
    eachrow.innerHTML = `
    <div  class="profile-pic-container2">
    <a href='/${comment.commenter.username}'>
                                                                <img class="profile-pic2" src="${comment.commenter.profile_pic}" >
                                                            </a>
