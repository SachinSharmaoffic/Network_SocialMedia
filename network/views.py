...
def edit_profile(request):
    if request.method == "POST":
        ...
        if request.FILES.get("profile"):
            request.user.profile_pic = request.FILES.get("profile")
        if request.FILES.get("cover"):
            request.user.cover = request.FILES.get("cover")
        request.user.save()
        return HttpResponseRedirect(reverse("index"))
    else:
        return render(request, "network/edit_profile.html", {"user": request.user})
